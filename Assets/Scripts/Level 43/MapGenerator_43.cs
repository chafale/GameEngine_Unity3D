...
public class MapGenerator_43 : MonoBehaviour {
	...
	GameObject GenerateCharacter(float start, float end, float obj1, float obj2, float obj3, float obj4) {
		...
		else if((count==2 || count%5==0) && GameManager_43.hints>0 && GameManager_43.hints<=3){
		...
		GameManager_43 gameMananger = GameObject.Find("GameManager").GetComponent<GameManager_43>();
